...
# Run anomaly detection once and store in session state
if "anomaly_results" not in st.session_state:
    st.session_state.anomaly_results = detect_anomalies()

time_test, anomaly_scores, anomalies, iso_flags, oc_flags, kmeans_flags, auto_flags = st.session_state.anomaly_results
...
st.write(f"🔍 **Anomalies Detected:** {int(sum(technique_map[technique]))}")
...
fig.add_trace(go.Scatter(
    x=time_test[technique_map[technique] == 1],
    y=anomaly_scores[technique_map[technique] == 1],
    mode='markers',
...
anomaly_df = pd.DataFrame({"Time": time_test, "Anomaly": technique_map[technique]})
heatmap_fig = px.scatter(
    anomaly_df, x="Time", y=technique_map[technique],
    color=technique_map[technique], color_continuous_scale="reds",
...
if st.checkbox("📜 Show Detailed Anomalies"):
    detailed_anomalies = pd.DataFrame({
        "Time": time_test[technique_map[technique] == 1],
        "Anomaly Score": anomaly_scores[technique_map[technique] == 1]
    })
    st.dataframe(detailed_anomalies.style.highlight_max(axis=0))

# Summary Statistics
st.sidebar.subheader("📊 Anomaly Summary")
st.sidebar.metric("Total Anomalies", int(sum(technique_map[technique])))
